# 1 "/tmp/license_clear.cpp.inc.31467.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 167 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/tmp/license_clear.cpp.inc.31467.cpp" 2
...
long long int p = 101*4;
long long int q = 103*3;
...
# 3 "/tmp/license_clear.cpp.inc.31467.cpp" 2
...
int main(int argc, char **argv) {
    ...
    for (i=Encoded(0); argv[1][(int)Encoded::DECODE(i)] != '\0'; i++) {
        day1 = Encoded(10) * day1 + Encoded((char)argv[1][Encoded::DECODE(i)]) - Encoded('0');
    }

    for (i=Encoded(0); argv[2][(int)Encoded::DECODE(i)] != '\0'; i++) {
        mon1 = Encoded(10) * mon1 + Encoded((char)argv[2][Encoded::DECODE(i)]) - Encoded('0');
    }

    for (i=Encoded(0); (int)Encoded::DECODE(i)<4; i++)
        year1 = Encoded(10) * year1 + Encoded((char)argv[3][Encoded::DECODE(i)]) - Encoded('0');


    time_t t = time(0);
    struct tm tm = *localtime(&t);

    day2 = Encoded(tm.tm_mday);
    mon2 = Encoded(tm.tm_mon) + Encoded(1);
    year2 = Encoded(tm.tm_year) + Encoded(1900);
    ...
    dd1 = Encoded(0);
    dd1 = Encoded(days_at_month(Encoded::DECODE(mon1)));

    for (i = ref; (int)Encoded::DECODE(i) < (int)Encoded::DECODE(year1); i++) {
        if ((int)Encoded::DECODE(i) % 4 == 0)
            dd1 += Encoded(1);
    }

    dd1 = dd1 + day1 + (year1 - ref) * Encoded(365);



    dd2 = Encoded(0);
    for (i = ref; (int)Encoded::DECODE(i) < (int)Encoded::DECODE(year2); i++) {
        if ((int)Encoded::DECODE(i) % 4 == 0)
            dd2 += Encoded(1);
    }

    dd2 = Encoded() + dd2 + day2 + ((year2 - ref) * Encoded(365));



    if ((int)Encoded::DECODE(dd2) - (int)Encoded::DECODE(dd1) > 30)
        printf("Yes\n", (int)Encoded::DECODE(dd2)-(int)Encoded::DECODE(dd1));
...
int days_at_month(int x)
{
    ...
    switch (x) {
        ...
        case 4: y = Encoded(90);
            break;
        case 5: y = Encoded(120);
            break;
        case 6: y = Encoded(151);
            break;
        case 7: y = Encoded(181);
            break;
        case 8: y = Encoded(212);
            break;
        case 9: y = Encoded(243);
            break;
        case 10:y = Encoded(273);
            break;
        case 11:y = Encoded(304);
            break;
        case 12:y = Encoded(334);
            break;
        default: printf("Error encountered");
            exit(1);
    }

    return (int)Encoded::DECODE(y);
}
